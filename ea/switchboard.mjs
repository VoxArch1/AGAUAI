...
export class Switchboard {
  ...
  runDAMN({ topic = null, deadlineMinutes = 10 } = {}) {
    ...
    const top = scored[0];
    const dissent = scored.slice(1).map(({ p, score }) => ({ id: p.id, agent: p.header.agent_id, score, summary: p.body.goal || p.body.approach || "" }));
  ...
  #moderate(p) {
    ...
    // external moderators
    for (const mod of this.moderators) { try { if (!mod(p)) return false; } catch { return false; } }
    return true;
  }
  ...
  #mediate(list) {
    ...
    for (const p of list) {
      ...
      if (!byGoal.has(key)) { byGoal.set(key, p); continue; }
  ...
  #score(p) {
    ...
    return +(wAgent + evidence + etaScore + domainBonus - riskPenalty).toFixed(4);
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (decision.payload?.[0]) console.log(`[EA] Top goal: ${decision.payload[0].plan.body.goal}`);
}
